import { useState, useEffect, useRef } from 'react';
import { useNavigate } from 'react-router-dom';
import { collection, getDocs, query, where } from 'firebase/firestore';
import { db } from '../services/firebase';
import { loadModels, getFaceDescriptor, compareFaces } from '../services/faceService';
import attendanceStore from '../services/attendanceStore';
import { createAttendanceSession } from '../services/qrService';
import { useAuth } from '../context/AuthContext';
import DashboardLayout from '../components/layout/DashboardLayout';
import { ScanFace, Users, CheckCircle2, Loader, AlertCircle, WifiOff, Play } from 'lucide-react';

// How long to show the welcome card before resetting (ms)
const WELCOME_DURATION = 3000;
// How often to run face detection (ms)
const SCAN_INTERVAL = 1200;
// Match threshold — lower = stricter
const THRESHOLD = 0.65;

const FaceKiosk = () => {
  const navigate = useNavigate();
  const videoRef = useRef(null);
  const streamRef = useRef(null);
  const intervalRef = useRef(null);
  const markedRef = useRef(new Set()); // UIDs marked this session (prevent duplicates)

  const [status, setStatus] = useState('init'); // init | loading | ready | error | no-session
  const [statusMsg, setStatusMsg] = useState('');
  const [session, setSession] = useState(null);
  const [students, setStudents] = useState([]);
  const [presentCount, setPresentCount] = useState(0);
  const [recentScans, setRecentScans] = useState([]);
  const [welcome, setWelcome] = useState(null);
  const [scanning, setScanning] = useState(false);
  // For creating a session inline
  const [newSubject, setNewSubject] = useState('');
  const [creating, setCreating] = useState(false);
  const { user } = useAuth();

  // ── Boot sequence ──────────────────────────────────────────────────────────
  useEffect(() => {
    let cancelled = false;
    const boot = async () => {
      setStatus('loading');
      setStatusMsg('Loading face recognition models...');
      try {
        await loadModels();
      } catch {
        setStatus('error');
        setStatusMsg('Failed to load face models. Refresh the page.');
        return;
      }
      if (cancelled) return;

      setStatusMsg('Finding active session...');
      const activeSession = await getActiveSession();
      if (!activeSession) {
        setStatus('no-session');
        setStatusMsg('No active session found. Start a session from the Attendance page first.');
        return;
      }
      if (cancelled) return;
      setSession(activeSession);

      // Seed already-marked UIDs so we don't double-mark
      (activeSession.attendees || []).forEach(a => markedRef.current.add(a.studentUID));
      setPresentCount(markedRef.current.size);

      setStatusMsg('Loading student face data...');
      const loaded = await loadStudentDescriptors();
      if (cancelled) return;
      const withFace = loaded.filter(s => s.descriptor);
      setStudents(withFace);

      if (withFace.length === 0) {
        setStatus('error');
        setStatusMsg('No students have registered their face yet. Ask students to register via /scan first.');
        return;
      }

      setStatusMsg('Starting camera...');
      const ok = await startCamera();
      if (!ok || cancelled) return;

      setStatus('ready');
      setStatusMsg('');
      startDetection(withFace, activeSession);
    };
    boot();
    return () => {
      cancelled = true;
      stopAll();
    };
  }, []);

  // ── Create session inline ──────────────────────────────────────────────────
  const handleCreateSession = async () => {
    if (!newSubject.trim()) return;
    setCreating(true);
    try {
      const s = await createAttendanceSession({
        subject: newSubject.trim(),
        section: '605A',
        teacher: user?.name || user?.email || 'Faculty',
        room: 'N/A',
      });
      setSession(s);
      setStatus('loading');
      setStatusMsg('Loading student face data...');
      const loaded = await loadStudentDescriptors();
      const withFace = loaded.filter(st => st.descriptor);
      setStudents(withFace);
      if (withFace.length === 0) {
        setStatus('error');
        setStatusMsg('No students have registered their face yet.');
        return;
      }
      setStatusMsg('Starting camera...');
      const ok = await startCamera();
      if (!ok) return;
      setStatus('ready');
      setStatusMsg('');
      startDetection(withFace, s);
    } catch (e) {
      console.error(e);
      setStatus('no-session');
      setStatusMsg('Failed to create session. Try again.');
    }
    setCreating(false);
  };
  const getActiveSession = async () => {
    try {
      // Try status=active first (no orderBy to avoid index requirement)
      const q = query(collection(db, 'sessions'), where('status', '==', 'active'));
      const snap = await getDocs(q);
      if (!snap.empty) {
        // Pick the most recently created one
        const sorted = snap.docs.sort((a, b) => {
          const aTime = a.data().createdAt?.seconds || 0;
          const bTime = b.data().createdAt?.seconds || 0;
          return bTime - aTime;
        });
        return { id: sorted[0].id, ...sorted[0].data() };
      }
      // Fallback: grab the most recent session regardless of status
      const allSnap = await getDocs(collection(db, 'sessions'));
      if (allSnap.empty) return null;
      const sorted = allSnap.docs.sort((a, b) => {
        const aTime = a.data().createdAt?.seconds || 0;
        const bTime = b.data().createdAt?.seconds || 0;
        return bTime - aTime;
      });
      return { id: sorted[0].id, ...sorted[0].data() };
    } catch (e) {
      console.error('getActiveSession error:', e);
      return null;
    }
  };

  const loadStudentDescriptors = async () => {
    const snap = await getDocs(collection(db, 'students'));
    return snap.docs.map(d => {
      const data = d.data();
      return {
        docId: d.id,
        uid: data.uid,
        name: data.name,
        section: data.section,
        descriptor: data.faceDescriptor ? new Float32Array(data.faceDescriptor) : null,
      };
    });
  };

  const startCamera = async () => {
    try {
      const stream = await navigator.mediaDevices.getUserMedia({
        video: { facingMode: 'user', width: { ideal: 640 }, height: { ideal: 480 } },
      });
      streamRef.current = stream;
      if (videoRef.current) {
        videoRef.current.srcObject = stream;
        await new Promise(r => { videoRef.current.onloadedmetadata = r; });
        videoRef.current.play();
      }
      return true;
    } catch {
      setStatus('error');
      setStatusMsg('Camera permission denied. Allow camera access and refresh.');
      return false;
    }
  };

  const stopAll = () => {
    clearInterval(intervalRef.current);
    if (streamRef.current) {
      streamRef.current.getTracks().forEach(t => t.stop());
      streamRef.current = null;
    }
  };

  // ── Face detection loop ────────────────────────────────────────────────────
  const startDetection = (studentList, activeSession) => {
    setScanning(true);
    intervalRef.current = setInterval(async () => {
      if (!videoRef.current) return;
      const descriptor = await getFaceDescriptor(videoRef.current);
      if (!descriptor) return;

      // Find best match
      let bestMatch = null;
      let bestDist = Infinity;
      for (const student of studentList) {
        if (!student.descriptor) continue;
        const { distance } = compareFaces(descriptor, student.descriptor, THRESHOLD);
        if (distance < bestDist) {
          bestDist = distance;
          bestMatch = student;
        }
      }

      if (bestMatch && bestDist < THRESHOLD) {
        if (markedRef.current.has(bestMatch.uid)) return; // already marked
        markedRef.current.add(bestMatch.uid);

        // Mark attendance in Firestore
        await attendanceStore.markAttendance(activeSession.id || activeSession.sessionId, {
          studentId: bestMatch.docId,
          studentName: bestMatch.name,
          studentUID: bestMatch.uid,
          section: bestMatch.section,
        });

        setPresentCount(markedRef.current.size);
        setRecentScans(prev => [{
          uid: bestMatch.uid,
          name: bestMatch.name,
          time: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
          dist: bestDist.toFixed(3),
        }, ...prev].slice(0, 8));

        // Show welcome card
        setWelcome({ name: bestMatch.name, uid: bestMatch.uid });
        setTimeout(() => setWelcome(null), WELCOME_DURATION);
      }
    }, SCAN_INTERVAL);
  };

  // ── Render ─────────────────────────────────────────────────────────────────
  return (
    <DashboardLayout>
      <div className="p-6 max-w-5xl mx-auto">
        {/* Header */}
        <div className="flex items-center justify-between mb-6">
          <div>
            <h1 className="text-2xl font-bold text-gray-900 flex items-center gap-2">
              <ScanFace size={28} className="text-[#1E3A8A]" />
              Face Attendance Kiosk
            </h1>
            {session && (
              <p className="text-sm text-gray-500 mt-1">
                {session.subject} · Section {session.section} · {session.teacher}
              </p>
            )}
          </div>
          <div className="flex items-center gap-3">
            {status === 'ready' && (
              <span className="flex items-center gap-1.5 text-sm text-emerald-700 bg-emerald-50 border border-emerald-200 px-3 py-1.5 rounded-full">
                <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" />
                Live
              </span>
            )}
            <button
              onClick={() => navigate('/attendance')}
              className="text-sm text-gray-500 hover:text-gray-700 border border-gray-200 px-3 py-1.5 rounded-lg"
            >
              ← Back
            </button>
          </div>
        </div>

        {/* Loading / Error states */}
        {status !== 'ready' && (
          <div className="flex flex-col items-center justify-center py-20 text-center">
            {status === 'loading' && <Loader size={40} className="text-[#1E3A8A] animate-spin mb-4" />}
            {status === 'error' && <AlertCircle size={40} className="text-red-500 mb-4" />}
            {status === 'no-session' && <WifiOff size={40} className="text-amber-500 mb-4" />}
            <p className="text-gray-700 font-medium max-w-sm">{statusMsg}</p>
            {status === 'no-session' && (
              <div className="mt-6 w-full max-w-sm">
                <p className="text-sm text-gray-500 mb-2 text-center">Or start a new session here:</p>
                <div className="flex gap-2">
                  <input
                    type="text"
                    value={newSubject}
                    onChange={e => setNewSubject(e.target.value)}
                    placeholder="Subject name (e.g. DBMS)"
                    className="flex-1 px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#1E3A8A]/20"
                    onKeyDown={e => e.key === 'Enter' && handleCreateSession()}
                  />
                  <button
                    onClick={handleCreateSession}
                    disabled={creating || !newSubject.trim()}
                    className="px-4 py-2 bg-[#1E3A8A] text-white rounded-lg text-sm disabled:opacity-50 flex items-center gap-1"
                  >
                    {creating ? <Loader size={14} className="animate-spin" /> : <Play size={14} />}
                    Start
                  </button>
                </div>
              </div>
            )}
          </div>
        )}

        {/* Main kiosk UI */}
        {status === 'ready' && (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            {/* Camera feed */}
            <div className="lg:col-span-2">
              <div className="relative rounded-2xl overflow-hidden bg-black shadow-lg" style={{ aspectRatio: '4/3' }}>
                <video
                  ref={videoRef}
                  autoPlay
                  muted
                  playsInline
                  className="w-full h-full object-cover"
                  style={{ transform: 'scaleX(-1)' }}
                />
                {/* Oval guide */}
                <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                  <div className="w-56 h-72 border-4 border-white/50 rounded-full" />
                </div>
                {/* Welcome overlay */}
                {welcome && (
                  <div className="absolute inset-0 flex items-center justify-center bg-black/60 backdrop-blur-sm">
                    <div className="text-center text-white px-6">
                      <CheckCircle2 size={64} className="text-emerald-400 mx-auto mb-3" />
                      <p className="text-3xl font-bold mb-1">Welcome!</p>
                      <p className="text-xl font-semibold text-emerald-300">{welcome.name}</p>
                      <p className="text-sm text-white/60 mt-1">{welcome.uid}</p>
                      <p className="text-sm text-emerald-400 mt-2">Attendance marked ✓</p>
                    </div>
                  </div>
                )}
                {/* Scanning indicator */}
                {scanning && !welcome && (
                  <div className="absolute bottom-3 left-1/2 -translate-x-1/2 flex items-center gap-2 bg-black/50 text-white text-xs px-3 py-1.5 rounded-full">
                    <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-pulse" />
                    Scanning for faces...
                  </div>
                )}
              </div>
              <p className="text-xs text-center text-gray-400 mt-2">
                Students: stand in front of the camera · Attendance marks automatically
              </p>
            </div>

            {/* Side panel */}
            <div className="space-y-4">
              {/* Stats */}
              <div className="bg-white border border-gray-200 rounded-xl p-4">
                <div className="flex items-center gap-3 mb-3">
                  <div className="w-10 h-10 bg-blue-50 rounded-lg flex items-center justify-center">
                    <Users size={20} className="text-[#1E3A8A]" />
                  </div>
                  <div>
                    <p className="text-2xl font-bold text-gray-900">{presentCount}</p>
                    <p className="text-xs text-gray-500">students marked present</p>
                  </div>
                </div>
                <div className="w-full bg-gray-100 rounded-full h-2">
                  <div
                    className="bg-[#1E3A8A] h-2 rounded-full transition-all duration-500"
                    style={{ width: `${Math.min((presentCount / 35) * 100, 100)}%` }}
                  />
                </div>
                <p className="text-xs text-gray-400 mt-1 text-right">{presentCount}/35</p>
              </div>

              {/* Registered faces count */}
              <div className="bg-white border border-gray-200 rounded-xl p-4">
                <p className="text-xs text-gray-500 mb-1">Faces loaded</p>
                <p className="text-lg font-semibold text-gray-800">{students.length} students</p>
                <p className="text-xs text-gray-400">with registered face data</p>
              </div>

              {/* Recent scans */}
              <div className="bg-white border border-gray-200 rounded-xl p-4">
                <p className="text-sm font-semibold text-gray-700 mb-3 flex items-center gap-2">
                  <CheckCircle2 size={16} className="text-emerald-500" />
                  Recent Scans
                </p>
                {recentScans.length === 0 ? (
                  <p className="text-xs text-gray-400 text-center py-4">No scans yet</p>
                ) : (
                  <div className="space-y-2">
                    {recentScans.map((s, i) => (
                      <div key={i} className="flex items-center justify-between text-sm">
                        <div>
                          <p className="font-medium text-gray-800">{s.name}</p>
                          <p className="text-xs text-gray-400">{s.uid}</p>
                        </div>
                        <div className="text-right">
                          <p className="text-xs text-gray-500">{s.time}</p>
                          <p className="text-xs text-emerald-600">✓</p>
                        </div>
                      </div>
                    ))}
                  </div>
                )}
              </div>
            </div>
          </div>
        )}
      </div>
    </DashboardLayout>
  );
};

export default FaceKiosk;
